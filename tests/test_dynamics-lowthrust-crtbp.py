...
from medusa.dynamics.lowthrust.crtbp import DynamicsModel, State
...
class TestDynamicsModel:
    ...
    def test_propagation(self, model, grp):
        y0 = State(model, [0.8213, 0.0, 0.5690, 0.0, -1.8214, 0.0])
        if len(grp) > 1:
            y0.fillDefaultICs(grp[1:])
    ...
    def test_checkPartials(self, model):
        y0 = State(model, [0.8213, 0.0, 0.5690, 0.0, -1.8214, 0.0])
        y0.fillDefaultICs(State.ALL_VARS[1:])
        tspan = [1.0, 1.1]
...
class TestState:
    ...
    def test_groupSize(self, model, grp, expect):
        ...
        assert state.groupSize(grp) == expect
