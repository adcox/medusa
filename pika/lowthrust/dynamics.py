...
class LowThrustCrtbpDynamics(CrtbpDynamics):
    ...
    def stateSize(self, varGroups):
        ...
        nCtrlState = self.ctrlLaw.numCtrlStats()
        nCtrlParam = len(self.ctrlLaw.params)

        return (6 + nCtrlState) * (VarGroups.STATE in varGroups) + (
            36 + nCtrlState * nCtrlState
        ) * (VarGroups.STM in varGroups)
        # TODO need to add epoch and param partials if control law defines them
    ...
    def diffEqs(self, t, q, varGroups, params=None):
        ...
        qdot[3:6] += self.ctrlLaw.accelVec(t, q, varGroups, params)

        count = 6  # track number of equations

        nCtrl = self.ctrlLaw.numCtrlStates
        nCore = count
        ...
        if VarGroups.EPOCH_PARTIALS in varGroups:
